package dev.circuitverse.game.core.engine.math;
...
public class Matrix4f {
    ...
    public static Matrix4f transform(Vector3f position, Vector3f rotation, Vector3f scale) {
        ...
        Matrix4f rotYMatrix = Matrix4f.rotate(rotation.getX(), new Vector3f(0, 1, 0));
        Matrix4f rotZMatrix = Matrix4f.rotate(rotation.getX(), new Vector3f(0, 0, 1));
    ...
    public static Matrix4f multiply(Matrix4f maxtrix, Matrix4f other) {
        Matrix4f result = Matrix4f.identity();

        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                result.set(i, j, maxtrix.get(i, 0) * other.get(0, j) +
                        maxtrix.get(i, 1) * other.get(1, j) +
                        maxtrix.get(i, 2) * other.get(2, j) +
                        maxtrix.get(i, 3) * other.get(3, j));
            }
        }
